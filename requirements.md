...
- The system shall allow a user's credential to be declared directly (an inline Argon2id hash, or one read from a file such as a mounted Docker/Kubernetes secret) as an alternative to setting it via the CLI or self-service reset, so config-managed accounts (LDAP service/bind accounts) need no imperative bootstrap step; such an account's credential shall be config-authoritative (Postgres bypassed at login, self-service reset and the CLI's set-password refused) and shall hot-reload the same as the rest of the declarative identity, including when only the referenced secret file's content changes.
